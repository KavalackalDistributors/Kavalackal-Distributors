...
function updateCategoryEditIcons() {
    ...
    // Add/remove Add Category button
...
// Enhanced displayProductsByCategory to handle dynamic categories
function displayProductsByCategory() {
    ...
    categoryKeys.forEach(catKey => {
        const categoryProducts = Array.isArray(products[catKey]) ? products[catKey] : [];
        // Create category heading always
        const heading = document.createElement('h2');
        heading.classList.add('section-heading');
        heading.textContent = getStoredCategoryName(catKey, catKey.charAt(0).toUpperCase() + catKey.slice(1) + ' Products');
        heading.setAttribute('data-cat-key', catKey);
        mainContainer.appendChild(heading);

        // Create container for products with navigation (can be empty)
        const container = createProductRow(categoryProducts, catKey);
...
// Enhanced showCategoryEditModal with delete functionality
function showCategoryEditModal(heading, catKey) {
    ...
    catEditModal.innerHTML = `
        <div class='category-edit-modal-content'>
            <h3>${categoryName}</h3>
            <button id='modal-edit-category-btn' class='modal-action-btn'>✏️ Rename Category</button>
            <button id='modal-add-product-btn' class='modal-action-btn'>➕ Add Product</button>
            <button id='modal-add-category-btn' class='modal-action-btn'>📂 Add Category</button>
            <button id='modal-edit-products-btn' class='modal-action-btn'>Edit Products</button>
            <button id='modal-delete-category-btn' class='modal-action-btn' style='background:#ff4444;color:white;'>🗑️ Delete Category</button>
            <button id='modal-cancel-category-btn' class='modal-action-btn' style='background:#ccc;color:#333;'>Cancel</button>
        </div>
    `;
    ...
    // Cancel closes modal
    catEditModal.querySelector('#modal-cancel-category-btn').onclick = function() {
        catEditModal.remove();
    };
    ...
    // Add Product inline from modal
    const addProductBtn = catEditModal.querySelector('#modal-add-product-btn');
    if (addProductBtn) {
        addProductBtn.onclick = function() {
            catEditModal.remove();
            showAddProductModal(catKey);
        };
    }

    // Add Category from modal
    const addCategoryBtn = catEditModal.querySelector('#modal-add-category-btn');
    if (addCategoryBtn) {
    ...
    catEditModal.querySelector('#modal-delete-category-btn').onclick = function() {
        catEditModal.remove();
        showDeleteCategoryModal(catKey, categoryName);
    };
}

// Show Delete Category Confirmation Modal
function showDeleteCategoryModal(catKey, categoryName) {
    ...
    deleteModal.className = 'category-edit-modal';
    deleteModal.style.display = 'flex';
    
    deleteModal.innerHTML = `
        <div class='category-edit-modal-content'>
            <h3>Delete Category</h3>
            <p style="margin: 1rem 0; color: #666;">Are you sure you want to delete "${categoryName}"?</p>
            <p style="margin-bottom: 1rem; color: #ff4444; font-size: 0.9rem;">This will remove all products from the website but won't delete images from GitHub.</p>
            <label for="admin-password">Admin Password:</label>
            <input type="password" id="admin-password" placeholder="Enter admin password" style="margin-bottom: 1rem;">
            <button id="confirm-delete-category-btn" class="modal-action-btn" style="background:#ff4444;color:white;">Delete Category</button>
            <button id="cancel-delete-category-btn" class="modal-action-btn" style="background:#ccc;color:#333;">Cancel</button>
        </div>
    `;
    ...
    // Cancel
    deleteModal.querySelector('#cancel-delete-category-btn').onclick = function() {
        deleteModal.remove();
    };
    
    // Confirm delete
    deleteModal.querySelector('#confirm-delete-category-btn').onclick = function() {
        const password = deleteModal.querySelector('#admin-password').value;
        const adminPassword = 'abysus@mj15'; // Admin password
        ...
        // Delete the category
        delete products[catKey];
        localStorage.removeItem('categoryName_' + catKey);
...
function showEditProductModal(catKey) {
	...
	if (!modal) return;
	let items = [];
	if (products[catKey] && Array.isArray(products[catKey])) {
		items = products[catKey];
	}
	...
	if (items.length > 0) fillFields(0);
	...
	const deleteBtn = modal.querySelector('#delete-edit-product-btn');
	if (deleteBtn) {
...
// Add Category modal (global function)
function showAddCategoryModal() {
	let addModal = document.getElementById('add-category-modal');
	if (addModal) addModal.remove();
	addModal = document.createElement('div');
	addModal.id = 'add-category-modal';
	addModal.className = 'category-edit-modal';
	addModal.style.display = 'flex';
	addModal.innerHTML = `
		<div class='category-edit-modal-content'>
			<h3>Add New Category</h3>
			<label for="category-key">Category Key (folder name):</label>
			<input type="text" id="category-key" placeholder="e.g., britannia" style="margin-bottom: 1rem;">
			<label for="category-display-name">Display Name:</label>
			<input type="text" id="category-display-name" placeholder="e.g., Britannia Products" style="margin-bottom: 1rem;">
			<button id="save-category-btn" class="modal-action-btn">Add Category</button>
			<button id="cancel-add-category-btn" class="modal-action-btn" style="background:#ccc;color:#333;">Cancel</button>
		</div>
	`;
	document.body.appendChild(addModal);
	addModal.querySelector('#cancel-add-category-btn').onclick = function() {
		addModal.remove();
	};
	addModal.querySelector('#save-category-btn').onclick = function() {
		const key = addModal.querySelector('#category-key').value.trim().toLowerCase();
		const displayName = addModal.querySelector('#category-display-name').value.trim();
		if (!key || !displayName) { 
		...
		products[key] = [];
		setStoredCategoryName(key, displayName);
		saveProductsToStorage();
		displayProductsByCategory();
		if (document.body.classList.contains('edit-mode')) {
			updateCategoryEditIcons();
...
function searchProducts(query) {
    ...
    Object.keys(products).forEach(catKey => {
        if (products[catKey] && Array.isArray(products[catKey])) {
            allProducts.push(...products[catKey]);
        }
    });
...
// Add Category Button - appears when Edit Website is enabled
function addAddCategoryButton() {
	// Remove any floating/nav buttons entirely (per request)
	const existingBtn = document.getElementById('add-category-btn');
	if (existingBtn) existingBtn.remove();
	const existingProdBtn = document.getElementById('add-product-btn');
	if (existingProdBtn) existingProdBtn.remove();
	// Nothing else to add here; buttons are now inside the edit (pencil) modal
}

function showAddProductModal(defaultCategoryKey) {
	let modal = document.getElementById('add-product-modal');
	if (modal) modal.remove();
	modal = document.createElement('div');
	modal.id = 'add-product-modal';
	modal.className = 'category-edit-modal';
	modal.style.display = 'flex';
	modal.innerHTML = `
		<div class='category-edit-modal-content'>
			<h3>Add Product</h3>
			...
			<button id="save-product-btn" class="modal-action-btn">Upload & Save</button>
			<button id="cancel-add-product-btn" class="modal-action-btn" style="background:#ccc;color:#333;">Cancel</button>
		</div>
	`;
	document.body.appendChild(modal);
	
	// Populate categories
	const select = modal.querySelector('#add-product-category');
	Object.keys(products).forEach(k => {
		const opt = document.createElement('option');
		opt.value = k;
		opt.textContent = getStoredCategoryName(k, k.charAt(0).toUpperCase() + k.slice(1) + ' Products');
		select.appendChild(opt);
	});
	if (defaultCategoryKey && select.querySelector(`option[value="${defaultCategoryKey}"]`)) {
		select.value = defaultCategoryKey;
	}
	
	modal.querySelector('#cancel-add-product-btn').onclick = () => modal.remove();
	
	modal.querySelector('#save-product-btn').onclick = async function() {
		const catKey = select.value;
		const name = modal.querySelector('#add-product-name').value.trim();
		const fileInput = modal.querySelector('#add-product-image');
		const file = fileInput.files && fileInput.files[0];
		if (!catKey || !name || !file) {
			alert('Please choose category, enter name, and select an image.');
			return;
		}
		...
		const nextId = (products[catKey]?.length || 0) + 1;
		const product = { id: nextId, name, image: dataUrl };
		if (!Array.isArray(products[catKey])) products[catKey] = [];
		products[catKey].push(product);
		saveProductsToStorage();
		displayProductsByCategory();
		updateCategoryEditIcons();
		modal.remove();
...
// Auto-Product Generator Helper Function
function generateProductsFromFilenames(categoryKey, filenames) {
    ...
    const newProducts = [];
    let nextId = products[categoryKey].length + 1;
    
    filenames.forEach(filename => {
        // Remove file extension and convert to product name
        ...
        const product = {
            id: nextId++,
            name: productName,
            image: `images/${categoryKey}/${filename}`,
            description: `Premium ${productName.toLowerCase()} from ${getStoredCategoryName(categoryKey, categoryKey)}.`
        };
    ...
    products[categoryKey] = [...products[categoryKey], ...newProducts];
    saveProductsToStorage();
    ...
    console.log(`Generated ${newProducts.length} products for category "${categoryKey}"`);
    return newProducts;
}

function removeProductsByNameExact(productName) {
	let removed = false;
	Object.keys(products).forEach(catKey => {
		if (Array.isArray(products[catKey])) {
			const before = products[catKey].length;
			products[catKey] = products[catKey].filter(p => (p?.name || '').trim() !== productName);
			if (products[catKey].length !== before) removed = true;
		}
	});
	if (removed) saveProductsToStorage();
}
...
function loadProductsFromStorage() {
	...
	if (stored) {
		try {
			...
			// Ensure every saved category exists; if new, create it
			Object.keys(parsed).forEach(categoryKey => {
				if (!Array.isArray(products[categoryKey])) {
					products[categoryKey] = [];
				}
				const savedArray = Array.isArray(parsed[categoryKey]) ? parsed[categoryKey] : [];
				// Merge by index, but also append any extra saved products
				savedArray.forEach((savedProduct, index) => {
					if (products[categoryKey][index]) {
						products[categoryKey][index] = { ...products[categoryKey][index], ...savedProduct };
					} else {
						products[categoryKey].push(savedProduct);
					}
				});
			});
		} catch (error) {
...
// Remove stray product named 'Aj' if it exists (per user request)
removeProductsByNameExact('Aj');
